...
### The problem
In autoregressive decoding, each new token attends to all previous tokens. Naively recomputing the key‑value (KV) pairs for every token would require $O(n^2)$ memory and $O(n^2)$ compute per step. But the KV pairs for earlier tokens are unchanged—they can be cached.
...
### The I/O bottleneck
Attention’s naive implementation reads/writes the entire $n \times n$ attention matrix from HBM, costing $O(n^2)$ bytes moved. On an A100, the compute‑to‑memory bandwidth ratio is ~200 FLOP/byte, meaning the operation is **memory‑bound** for all but tiny sequences.
...
### I/O complexity
If SRAM holds $M$ bytes, each tile is of size $B \times d$. Choosing $B \approx \sqrt{M/d}$ balances the number of $Q$ tiles and $K$ tiles. The total HBM traffic becomes $O(n^2 d / M)$ instead of $O(n^2)$. For typical $M = 20 \text{MB}$ and $d = 128$, this is a ~100× reduction in memory traffic.
